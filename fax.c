...
gint spandsp_init(const gchar *tiff_file, gboolean sending, gchar modem, gchar ecm, const gchar *lsi, const gchar *local_header_info, struct capi_connection *connection)
{
	...
	struct fax_status *status = (struct fax_status *)connection->priv;
	if (obverb) printf("Beginn spandsp_init\n");
...
gint spandsp_close(fax_state_t *fax_state)
{
	...
	if (fax_state != NULL) {
		...
		for (i = 0; i < CAPI_CONNECTIONS; i++) {
			status = (struct fax_status*)session->connection[i].priv;
...
//  printf("Beginn fax_transfer\n");
	...
	//isdn_unlock();
//  printf("Ende fax_transfer, %i/%i\n",((struct fax_status*)connection->priv)->bytes_sent,((struct fax_status*)connection->priv)->bytes_total);
}
...
void fax_clean(struct capi_connection *connection)
{
  ...
  if (obverb) printf("Beginn fax_clean\n");
	spandsp_close(status->fax_state);
  if (obverb) printf("Mitte fax_clean\n");

	free(status);
  if (obverb) printf("Ende fax_clean\n");
